...
const InputCurrency2 = ({
    ...
    const formatCurrency = (val, lang) => {
        const numericValue = val.replace(/[^\d]/g, '');
        if (numericValue === '') return '';
        
        const floatValue = parseFloat(numericValue) / Math.pow(10, decimalPlaces);

        switch(lang) {
            case 'es':
                const formattedNumber = floatValue.toFixed(decimalPlaces);
            ...
            default:
                console.warn('Invalid lang parameter. Using "normal" format.');
                return floatValue.toFixed(decimalPlaces).replace('.', ',');
        }
    };
    ...
    const handleInputChange = (e) => {
        ...
        // If the input is empty, set the value to empty string
        if (inputValue === '') {
            setInternalValue('');
            if (onChange) {
    ...
    // ... (rest of the component code, including style definitions)
    const heightMap = {
        inside: {
          sm: 'h-12',
          md: 'h-14',
        ...
        <div>
            ...
            <div onClick={handleDivClick} className={containerClassNames}>
                ...
                <div className={`flex w-full items-center h-full ${labelPlacement === 'inside' && 'translate-y-2'}`}>
                    ...
                    <div className='text-zinc-400 pe-2 select-none'>{currency}</div>
                    <input
                        {...inputProps}
                        autoComplete="off"
